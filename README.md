...
个人量化工具。**主线是一个五步作战闭环**（[`docs/21-battle-loop.md`](docs/21-battle-loop.md)）：

```
① 选标的（任意股票/场内ETF，检索即入库拉全量历史）
   ↓
② 复盘：调参数 → 跑全历史 → 看收益/回撤/买卖次数（同一引擎同一口径）
   ↓
③ 作战台：生成未来一周条件动作单 —— 收盘≤X买多少、≥Y卖多少份，
   每条规则附同参数的历史表现（规则触发单，不是价格预测）
   ↓
④ 我的盘：录入真实成交 → 计划 vs 实际逐日对照 → 反哺建议（档位过远/执行率/追高）
   ↓
⑤ 策略库：参数版本快照 + 血缘 + 同口径对比 → 回到 ② 迭代
```

界面四页：**作战台 / 复盘 / 我的盘 / 策略库**（旧研究页保留路由，退出主导航）。
...
以下是数据层建设过程中已验证的结论（研究过程见 docs/01~20）：
...
# 四页作战闭环：作战台（态势+未来一周条件动作单）/ 复盘（调参+回测+存版本）/
#       我的盘（实际盘录入+计划vs实际反哺）/ 策略库（版本+对比）
...
└── src/libre_quant/              # 库层（可 pip install；scripts 单向依赖它）
    ...
    ├── battle.py                作战方案：未来一周条件触发单 + 计划vs实际反哺（docs/21）
